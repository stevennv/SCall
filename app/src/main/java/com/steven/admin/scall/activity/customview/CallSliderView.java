package com.steven.admin.scall.activity.customview;
...
public class CallSliderView extends View {
    ...
    private void actionDown(MotionEvent event) {
        ...
        if (x >= circleOffset && x <= circleOffset + redRegionWidth && y >= 0
                && y <= height) { //只有点在不了滑动View上才可以滑动
            isDownRight = true;
            prevX = (int) event.getX();
            maxOffset = width / 2 - redRegionWidth / 2;
        } else {
            isDownRight = false;
            if (x > 0 && x < height && y >= 0 && y < height) { //点在了接听上 点击效果
                if (sliderListener != null) {
                    sliderListener.onSliderListen();
                }
            }
            if (x > width - height && x < width && y >= 0 && y < height) { //点在了挂断上 点击效果
                if (sliderListener != null) {
